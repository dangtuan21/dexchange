...
contract('Dex', (accounts) => {
  ...
  // it('7. should NOT create limit order if token balance too low', async () => {
  //   await dex.deposit(
  //     web3.utils.toWei('99'),
  //     REP, {
  //       from: trader1
...
//   it('8. should NOT create limit order if dai balance too low', async () => {
//     await dex.deposit(
//       web3.utils.toWei('99'),
//       DAI, {
//         from: trader1
...
//       err = error;      
//     }
//     assert(err.reason === 'dai balance too low');
...
//   it('9. should NOT create limit order if token is DAI', async () => {
//     let err;
//     try {
//       await dex.createLimitOrder(
//         DAI,
...
//       err = error;
//     }
//     assert(err.reason === 'cannot trade DAI');
...
// it('10 should NOT create limit order if token does not not exist', async () => {
//   await expectRevert(
//     dex.createLimitOrder(
...
it('11. should create market order & match', async () => {
  await dex.deposit(
    web3.utils.toWei('100'),
    DAI, {
      from: trader1
...
it('12. should NOT create market order if token balance too low', async () => {
  await expectRevert(
...
it('13. should NOT create market order if dai balance too low', async () => {
  await dex.deposit(
    web3.utils.toWei('100'),
    REP, {
      from: trader1
...
it('14. should NOT create market order if token is DAI', async () => {
  await expectRevert(
    dex.createMarketOrder(
      DAI,
      web3.utils.toWei('1000'),
...
it('15. should NOT create market order if token does not not exist', async () => {
  await expectRevert(
    dex.createMarketOrder(
      web3.utils.fromAscii('TOKEN-DOES-NOT-EXIST'),
      web3.utils.toWei('1000'),
